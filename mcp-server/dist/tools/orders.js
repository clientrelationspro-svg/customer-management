...
export const orderTools = [
    {
        ...
        handler: async (args) => {
            ...
            if (args.search)
                where.orderNo = { contains: args.search };
            ...
            const results = orders.map((o) => ({
                ID: o.id,
                订单号: o.orderNo,
                客户: o.customer.companyName,
                状态: o.status,
                总金额: formatCurrency(o.totalAmount),
                已付金额: formatCurrency(o.paidAmount),
                产品数: o.items.length,
                产品: o.items.map((i) => `${i.product.name} x${i.quantity}`),
                订单日期: formatDate(o.orderDate),
                交付日期: formatDate(o.deliveryDate),
                备注: o.notes || '—',
            }));
            return {
                content: [{
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            if (!order)
                return { content: [{ type: 'text', text: `❌ 未找到订单 ID: ${args.orderId}` }] };
            const info = {
                ID: order.id,
                订单号: order.orderNo,
                ...
                备注: order.notes || '—',
                订单项: order.items.map((i) => ({
                    产品: i.product.name,
                    SKU: i.product.sku,
                    数量: i.quantity,
                    单价: formatCurrency(i.unitPrice),
                    小计: formatCurrency(i.totalPrice),
                })),
            };
                ...
                const order = await prisma.$transaction(async (tx) => {
                    ...
                    for (const item of args.items) {
                        ...
                        if (product.stock < item.quantity)
                            throw new Error(`产品 ${product.name} 库存不足 (当前: ${product.stock}, 需要: ${item.quantity})`);
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                return { content: [{ type: 'text', text: `✅ 订单 ${order.orderNo} 状态已更新为: ${args.status}` }] };
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
