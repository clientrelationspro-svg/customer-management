...
export const productTools = [
    {
        ...
        handler: async (args) => {
            ...
            const results = products.map((p) => ({
                ...
                SKU: p.sku,
                分类: p.category || '—',
                售价: formatCurrency(p.price),
                成本: formatCurrency(p.cost),
                库存: p.stock,
                最低库存: p.minStock,
                状态: p.status,
                库存预警: p.stock <= p.minStock ? '⚠️ 库存不足' : '✅ 正常',
                供应商: p.supplier?.name || '—',
                描述: p.description || '—',
            }));
            return {
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            if (!product)
                return { content: [{ type: 'text', text: `❌ 未找到产品 ID: ${args.productId}` }] };
            const info = {
                ID: product.id,
                名称: product.name,
                SKU: product.sku,
                ...
                最近订单: product.orderItems.map((oi) => ({
                    订单号: oi.order.orderNo, 客户: oi.order.customer.companyName, 日期: formatDate(oi.order.orderDate),
                })),
            };
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                return { content: [{ type: 'text', text: `✅ 产品 ${product.name} (${product.sku}) 库存已更新为: ${args.stock}` }] };
            }
            catch (error) {
