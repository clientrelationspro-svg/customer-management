...
export const customerTools = [
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            if (!customer) {
                return { content: [{ type: 'text', text: `❌ 未找到客户 ID: ${args.customerId}` }] };
            }
            const info = {
                ID: customer.id,
                ...
                最近订单: customer.orders.map((o) => ({
                    订单号: o.orderNo, 状态: o.status, 金额: o.totalAmount?.toString() || '—', 日期: formatDate(o.orderDate),
                })),
                最近跟进: customer.followUps.map((f) => ({
                    事项: f.followUpMatters, 方式: f.contactMethod, 优先级: f.priority, 阶段: f.stage || '—', 状态: f.status, 备注: truncate(f.remarks, 80),
                })),
                需求分析: customer.needs.map((n) => ({
    ...
    },
    {
        ...
        handler: async (args) => {
            try {
                ...
                return { content: [{ type: 'text', text: `✅ 客户 ${args.customerId} 已删除（含关联数据）` }] };
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                return { content: [{ type: 'text', text: `❌ 删除失败: ${message}` }] };
            }
        },
    },
];
