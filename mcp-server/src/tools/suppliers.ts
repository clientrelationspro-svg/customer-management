...
export const supplierTools = [
    ...
    handler: async (args: Record<string, unknown>) => {
      ...
      if (args.search) {
        where.OR = [
          ...
          { country: { contains: args.search as string } },
          { mainProducts: { contains: args.search as string } },
        ];
      }
      if (args.cooperationStatus) where.cooperationStatus = args.cooperationStatus;
      if (args.riskLevel) where.riskLevel = args.riskLevel;
      if (args.starred) where.isStarred = true;
      ...
      const results = suppliers.map((s: Record<string, unknown>) => ({
        ID: s.id,
        名称: s.name,
        国家: s.country || '—',
        主要产品: s.mainProducts || '—',
        合作状态: statusMap[s.cooperationStatus as string] || s.cooperationStatus,
        风险等级: s.riskLevel ? riskMap[s.riskLevel as string] || s.riskLevel : '—',
        收藏: s.isStarred ? '⭐' : '',
        邮箱: s.email || '—',
        电话: s.phone || '—',
        产品数: (s._count as Record<string, number>).products,
        联系人数: (s._count as Record<string, number>).contacts,
      }));
  ...
  },
  {
    ...
    handler: async (args: { supplierId: string }) => {
      ...
      if (!supplier) return { content: [{ type: 'text' as const, text: `❌ 未找到供应商 ID: ${args.supplierId}` }] };
      ...
      const info: Record<string, unknown> = {
        ...
        联系人: supplier.contacts.map((c: Record<string, unknown>) => ({
          姓名: c.name, 职位: c.position || '—', 邮箱: c.email || '—', 电话: c.phone || '—',
          决策权重: c.decisionWeight || '—', 沟通偏好: c.communicationPreference || '—',
        })),
        产品: supplier.products.map((p: Record<string, unknown>) => ({ 名称: p.name, SKU: p.sku, 价格: (p.price as { toString: () => string })?.toString() || '—' })),
        风险事件: supplier.riskEvents.map((e: Record<string, unknown>) => ({
          类型: e.riskType, 描述: e.description, 严重度: e.severity,
          发生时间: formatDate(e.occurredAt as string), 解决时间: formatDate(e.resolvedAt as string),
        })),
      };
