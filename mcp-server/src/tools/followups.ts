...
export const followUpTools = [
  {
    ...
    handler: async (args: Record<string, unknown>) => {
      ...
      const results = followUps.map((f: Record<string, unknown>) => {
        const isOverdue = f.nextFollowUpDate && new Date(f.nextFollowUpDate as string) < new Date();
        const customerInfo = f.customer as Record<string, unknown>;
        return {
          ID: f.id,
          客户: customerInfo.companyName,
          事项: f.followUpMatters,
          联系方式: f.contactMethod,
          优先级: priorityMap[f.priority as string] || f.priority,
          状态: f.status,
          阶段: f.stage || '—',
          最近跟进: formatDate(f.lastFollowUpDate as string),
          下次跟进: formatDate(f.nextFollowUpDate as string),
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            逾期数: args.overdue ? total : followUps.filter((f: Record<string, unknown>) => f.nextFollowUpDate && new Date(f.nextFollowUpDate as string) < new Date()).length,
            跟进列表: results,
          }, null, 2),
        }],
      };
      ...
      try {
        const followUp = await prisma.followUp.create({
          data: {
            ...
            followUpMatters: args.followUpMatters as string,
            contactMethod: args.contactMethod as string,
            priority: (args.priority as string) || 'medium',
            stage: args.stage as string | undefined,
            lastFollowUpDate: new Date(),
            nextFollowUpDate: args.nextFollowUpDate ? new Date(args.nextFollowUpDate as string) : undefined,
    ...
    handler: async (args: Record<string, unknown>) => {
      try {
        ...
        if (args.nextFollowUpDate) {
          updateData.nextFollowUpDate = new Date(args.nextFollowUpDate as string);
        }
        const followUp = await prisma.followUp.update({
          where: { id: followUpId as string },
          data: updateData,
        });
        return { content: [{ type: 'text' as const, text: `✅ 跟进记录 ${followUp.id} 已更新` }] };
      } catch (error: unknown) {
        const message = error instanceof Error ? error.message : String(error);
