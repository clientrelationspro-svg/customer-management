...
import { Sparkles, Copy, Download, Upload, X, ChevronDown, ChevronUp, Loader2, Trash2, Settings2, Mail, FileText, Target, BookOpen } from 'lucide-react';
...
export default function SequencePromptBuilder({ customerId, inquirySubject, inquiryBody, inquiryId, onImport }: Props) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const d = await r.json();
      if (d.success && d.data) d.data.forEach((f: any) => {
        if (f.remarks) {
  ...
  const buildPrompt = () => {
    ...
    const t1 = t.toISOString().split('T')[0];

    const p = `你是资深外贸业务专家，以 **${roleLabels[role]}** 身份生成定时跟进邮件序列。
${dataBlock}${emailBlock}
## ⚙️ 参数
...
--- 第1封 ---
subject: "[主题]"
scheduledAt: "${t1} 09:00"
content: |
  [Markdown正文，约${words}字]

...共${count}封，subject/scheduledAt/content 各占一行，字段间空行分隔
\`\`\`
禁止: 代码块标记、多余解释`;
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const r = await fetch(`/api/inquiries/${inquiryId}/follow-ups/import`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: importText }) });
      const d = await r.json();
      setImportResult(d.success ? `✅ 已导入 ${d.count} 封` : `❌ ${d.error}`);
      if (d.success) { setImportText(''); onImport?.(); }
            ...
            <div className="p-4 space-y-4">
              ...
              {prompt && (
                <textarea value={prompt} onChange={e => setPrompt(e.target.value)}
                  rows={12} className="w-full px-3 py-2 bg-gray-900 text-gray-100 rounded-lg text-xs font-mono resize-y border-0 focus:ring-2 focus:ring-purple-500" />
              )}
            </div>
          ) : (
            /* Import Tab */
            <div className="p-4 space-y-3">
              <p className="text-xs text-gray-500">粘贴 WorkBuddy / AI 生成的定时序列输出</p>
              ...
              <div className="flex items-center gap-2">
                <Button size="sm" onClick={handleImport} loading={importing}><Upload className="w-3 h-3 mr-1" />导入</Button>
